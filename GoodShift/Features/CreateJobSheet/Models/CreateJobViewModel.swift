...
class CreateJobViewModel {
	...
	/// Set when the displayed image is a bundled asset (not user-picked) — prevents unnecessary upload
	...
	/// Pass the already-resolved `serviceImage` (uploaded URL or asset name) so all shifts share the same reference.
	func createServices(category: ServiceCategoryType, user: User, serviceImage: ServiceImage) -> [JobService] {
