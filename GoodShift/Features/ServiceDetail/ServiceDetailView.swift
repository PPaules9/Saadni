...
struct ServiceDetailView: View {
				...
				VStack(alignment: .leading, spacing: 24) {
					...
					if !service.description.isEmpty {
						VStack(alignment: .leading, spacing: 8) {
							SectionHeader(title: "About This Job")
					...
					if let tools = service.specialTools, !tools.isEmpty {
						VStack(alignment: .leading, spacing: 8) {
							SectionHeader(title: "Tools & Equipment Needed")
	...
	private var pendingCompletionBanner: some View {
		StatusBannerCard(
			...
			subtitle: service.completionRequestedAt.map { "Submitted \($0.formatted(.relative(presentation: .named)))" },
			tint: .purple
		)
	}
	...
	private var disputedBanner: some View {
		StatusBannerCard(
			...
			subtitle: service.disputeReason.flatMap { $0.isEmpty ? nil : $0 },
			tint: .red
		)
	}
	...
	private var providerApplicantsSection: some View {
		...
		if !applications.isEmpty {
			VStack(alignment: .leading, spacing: 12) {
				SectionHeader(title: "Applicants (\(applications.count))")
        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 10) {
                    ...
                    // Personal info chips: age, gender, city
                    let ageChip = user?.age.map { ("\($0) yrs", "person") }
                    let genderChip: (String, String)? = user?.gender.map { g in
                        let label = g == "male" ? "Male" : g == "female" ? "Female" : "Other"
                        return (label, "person.fill")
                    }
                    let cityChip = user?.city.map { ($0, "mappin.circle.fill") }
                    let chips: [(String, String)] = [ageChip, genderChip, cityChip].compactMap { $0 }

                    if !chips.isEmpty {
                        HStack(spacing: 8) {
                            ForEach(chips, id: \.0) { label, icon in
                ...
                // MARK: - Strikes Warning
                if let strikes = user?.strikes, strikes > 0 {
                    HStack(spacing: 10) {
                        Image(systemName: "exclamationmark.triangle.fill")
                            .foregroundStyle(.orange)
                        Text(strikes == 1 ? "1 strike on record" : "\(strikes) strikes on record")
                ...
                // MARK: - Offered Services / Skills
                if let services = user?.offeredServices, !services.isEmpty {
                    VStack(alignment: .leading, spacing: 10) {
                        Label("Skills", systemImage: "wrench.and.screwdriver.fill")
                            .font(.subheadline.weight(.semibold))
                            .foregroundStyle(Colors.swiftUIColor(.textSecondary))
                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 90), spacing: 8)], spacing: 8) {
                            ForEach(services, id: \.self) { skill in
                                Text(skill)
                                    .font(.caption.weight(.medium))
                                    .lineLimit(1)
                                    .padding(.horizontal, 10).padding(.vertical, 5)
                                    .frame(maxWidth: .infinity)
                                    .background(Colors.swiftUIColor(.primary).opacity(0.12))
                ...
                // MARK: - Proposed Price
                if let proposedPrice = application.proposedPrice {
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Proposed Price")
                                .font(.caption.weight(.semibold))
                                .foregroundStyle(Colors.swiftUIColor(.textSecondary))
                            Text(String(format: "%.0f EGP", proposedPrice))
                    ...
                    .padding(14)
                    .background(Colors.swiftUIColor(.primary).opacity(0.08))
                    .cornerRadius(14)
                    .overlay(RoundedRectangle(cornerRadius: 14).strokeBorder(Colors.swiftUIColor(.primary).opacity(0.25), lineWidth: 1))
                }

                // MARK: - Application Meta
                VStack(spacing: 6) {
                    HStack(spacing: 8) {
                    ...
                    if let memberSince = memberSinceText {
                        HStack(spacing: 8) {
                            Image(systemName: "calendar").foregroundStyle(Colors.swiftUIColor(.textSecondary))
                            Text(memberSince)
                                .font(.caption)
                                .foregroundStyle(Colors.swiftUIColor(.textSecondary))
                            Spacer()
