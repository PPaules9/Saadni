...
struct GoodShiftApp: App {
	...
	private func setupNotificationDelegates() {
		...
		// Re-register on every launch so APNs token is always fresh and passed to Firebase.
		// onChange(of: hasSeenOnboarding) only fires on the first-time grant; returning users
		// would never call registerForRemoteNotifications() without this check.
